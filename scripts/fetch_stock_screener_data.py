...
Finance tickers). This mirrors fetch_breakout_scanner.py exactly (same scoring logic, same
batching/retry strategy) but scores individual stocks instead of ETFs, feeding the site's
Stock Screener tab (a duplicate of the ETF Screener tab, pointed at this data file instead).
...
# Same batch size/retry/pause settings as fetch_breakout_scanner.py, which pulls full OHLCV
# over a comparable (~1,026 vs ~966 ticker) universe reliably — no reason to tune these
# differently for this universe.
BATCH_SIZE = 30
RETRIES = 4
