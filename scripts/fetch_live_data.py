#!/usr/bin/env python3
"""
Refreshes everything on the site except the ETF heatmap (that's fetch_etf_data.py):
...
def fetch_nifty500_price(existing):
    try:
        h = yf.Ticker("^CRSLDX").history(period="max", interval="1d", auto_adjust=True)
        if len(h) == 0:
            return existing
        series = [[d.strftime("%Y-%m-%d"), round(float(c), 2)] for d, c in h["Close"].items()]
        print(f"  nifty500 price: {len(series)} rows")
        return series
    except Exception as e:
        print(f"  nifty500 price FAILED: {e}")
        return existing


def fetch_breadth(existing):
    # existing: [["YYYY-MM-DD", pct], ...] — only appends dates after the last saved one.
    last_date = existing[-1][0] if existing else "2006-10-20"
    try:
        ...
        print(f"  breadth: {len(symbols)} constituents")
    except Exception as e:
        print(f"  breadth constituent list FAILED, skipping breadth update: {e}")
        return existing

    import pandas as pd
    tickers = [s + ".NS" for s in symbols]
    try:
        raw = yf.download(tickers, period="300d", interval="1d", auto_adjust=True,
                           group_by="ticker", threads=True, progress=False)
    except Exception as e:
        print(f"  breadth price download FAILED: {e}")
        return existing
    ...
    for t in tickers:
        try:
            ...
            continue
    if not closes:
        print("  breadth: no usable price series, skipping")
        return existing
    ...
    if new_rows:
        print(f"  breadth: appending {len(new_rows)} new day(s)")
        return existing + new_rows
    print("  breadth: already up to date")
    return existing
