...
def fetch_index_price(ticker, existing, label):
    try:
        ...
        print(f"  {label} price: {len(series)} rows")
        return series
    except Exception as e:
        print(f"  {label} price FAILED: {e}")
        return existing
...
def fetch_breadth_generic(constituents_url, existing, default_last_date, label):
...
def main():
    ...
    sc250_breadth = fetch_breadth_generic(SC250_CONSTITUENTS_URL, sc250_breadth, "2024-01-01", "sc250")
